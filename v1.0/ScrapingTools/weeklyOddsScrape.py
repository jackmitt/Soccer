...
dateStart = datetime.date(2021, 1, 11)
dateThru = datetime.date(2021, 1, 15)
...
for row in rows:
    if (len(row.find_all(class_="first2 tl")) != 0):
        if (not "England" in row.find(class_="first2 tl").text):
            ...
            if (d.split()[0] == "Today," or d.split()[0] == "Tomorrow,"):
                #####################################################################################################CHANGE YEAR MANUALLY
                curDate = datetime.date(2021, monthToInt(d.split()[2]), int(d.split()[1]))
            else:
